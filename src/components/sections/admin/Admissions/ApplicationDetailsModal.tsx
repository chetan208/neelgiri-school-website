...
export default function ApplicationDetailsModal({ isOpen, onClose, data }: ApplicationDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden transform animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-teal-600 px-6 py-4 flex justify-between items-center text-white">
          <div>
            ...
            <p className="text-teal-100 text-xs mt-0.5">Application ID: {data.id}</p>
          </div>
          <button onClick={onClose} className="p-1 hover:bg-teal-700 border-0 bg-transparent cursor-pointer rounded-full transition-colors text-white">
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs font-semibold text-teal-700">{data.targetClass}</p>
            </div>
            <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs font-medium text-slate-800 bg-slate-50 p-2 rounded-lg border border-slate-100">{data.address}</p>
            </div>
            <div>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
          <span className={`px-3 py-1 text-[10px] font-black tracking-wider rounded-full ${data.status === 'PENDING' ? 'bg-amber-100 text-amber-700' : data.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
            Status: {data.status}
          </span>
