    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
      ...
      <div className="px-4 sm:px-6 py-3 border-b border-slate-100 bg-slate-50/50 flex flex-wrap justify-between items-center gap-2">
        ...
        <button 
          onClick={refreshData}
          disabled={isLoading}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 text-slate-600 hover:text-[#FA6781] rounded-lg text-xs font-semibold transition-all border-1 cursor-pointer disabled:opacity-50 font-sans"
        >
          <svg className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin text-[#FA6781]' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <td className="px-4 sm:px-6 py-4 text-right">
                    {type === 'pending' ? (
                      <div className="flex justify-end gap-1.5 flex-wrap sm:flex-nowrap">
                        <button onClick={(e) => { e.stopPropagation(); setModal({isOpen: true, id: item.id, status: 'APPROVED'}); }} className="px-2 py-1 border-0 cursor-pointer bg-[#59B292]/10 text-[#59B292] text-[9px] font-black rounded hover:bg-[#59B292]/20 transition-colors">APPROVE</button>
                      ...
                      <span className="text-[9px] font-black px-2 py-0.5 bg-[#59B292]/15 text-[#59B292] rounded">APPROVED</span>
                    )}
                  </td>
                </tr>
              ))
