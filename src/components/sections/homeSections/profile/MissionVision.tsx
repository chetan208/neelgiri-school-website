...
function Card({ type, icon: Icon, accentColor, iconBg, iconColor, tagBg, tagText, title, body, points, visible, delay }: CardProps) {
  ...
  return (
    <div
      ...
      className="group relative rounded-2xl overflow-hidden border border-slate-200 bg-white flex flex-col"
      style={{
        opacity: visible ? 1 : 0,
        transform: visible ? "translateY(0)" : "translateY(32px)",
        transition: `opacity 0.65s ease ${delay}ms, transform 0.65s ease ${delay}ms, box-shadow 0.3s ease`,
        boxShadow: hovered
          ? "0 20px 48px -12px rgba(109,40,217,0.16), 0 4px 12px -2px rgba(0,0,0,0.07)"
    ...
    >
      <div className="relative z-10 p-7 sm:p-8 flex flex-col h-full">
        ...
        <h3 className="text-xl sm:text-2xl font-black text-slate-800 mb-3 leading-snug">{title}</h3>
        ...
        <p className="text-slate-500 text-sm leading-relaxed mb-6 flex-grow">{body}</p>
        ...
        <ul className="space-y-3">
          {points.map((pt, i) => (
            ...
            >
              ...
              <span className="text-sm text-slate-600 font-medium">{pt}</span>
            </li>
          ))}
        </ul>
      </div>
...
export default function MissionVision() {
  ...
  return (
    <section className="relative overflow-hidden bg-slate-50 py-16 sm:py-24 px-4 sm:px-6">
      <div className="absolute top-[-80px] left-[-60px] w-80 h-80 rounded-full bg-violet-200 opacity-25 blur-[70px] pointer-events-none" />
      <div className="absolute bottom-[-60px] right-[-40px] w-72 h-72 rounded-full bg-teal-200 opacity-20 blur-[60px] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        <div ref={headerRef} className="flex flex-col items-center text-center mb-12">
          ...
          >
            <span className="inline-block text-[10px] font-bold tracking-[0.2em] uppercase px-3 py-1.5 rounded-full bg-violet-100 text-violet-700 mb-4">
              Our Foundation
            </span>
          </div>

          <h2
            className="text-4xl sm:text-5xl font-black text-slate-900 leading-tight tracking-tight mb-4"
            style={{
              opacity: headerVisible ? 1 : 0,
          ...
          >
            ...
            <span className="relative inline-block">
              <span className="relative z-10 text-violet-600">Vision</span>
              <svg
              ...
              >
                <path d="M2 8 C40 2, 80 9, 120 4 C160 -1, 180 8, 198 5"
                  stroke="#7C3AED" strokeWidth="2.5" strokeLinecap="round" fill="none" opacity="0.45" />
              </svg>
            </span>
          </h2>

          <p
            className="max-w-lg text-slate-500 text-sm sm:text-base leading-relaxed"
            style={{
              opacity: headerVisible ? 1 : 0,
